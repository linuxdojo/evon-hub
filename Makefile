...
deploy: # Build and deploy latest bootstrap.sh and deployment_mapper to evon.link
	# build and deploy bootstrap
	make bootstrap
	echo "Deploying bootstrap..."
	scp bootstrap.sh linuxdojo@evon.link:/tmp
	ssh linuxdojo@evon.link " \
	...
	scp /tmp/deployment_mapper.tgz linuxdojo@evon.link:/tmp
	...
	# done
	echo "Done: "
	echo "  - new bootstrap is now available at https://<username>:<password>@evon.link/bootstrap"
